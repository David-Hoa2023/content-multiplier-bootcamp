...
export default function DerivativesPage() {
  ...
  const selectAllDerivatives = () => {
    const allIds = derivatives.filter(d => d.id && (!d.status || d.status !== 'published')).map(d => d.id!)
              ...
              <CardContent>
                {derivatives.length > 0 ? (
                  <>
                    ...
                    <div className="p-2 bg-yellow-50 rounded text-xs mb-2 border">
                      Debug: {derivatives.length} derivatives total, 
                      {derivatives.filter(d => d.id).length} with IDs, 
                      {derivatives.filter(d => !d.status || d.status !== 'published').length} selectable
                    </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg mb-4">
                      <div className="flex items-center gap-4">
                        <div className="text-sm font-medium">
                          {selectedDerivatives.length} / {derivatives.filter(d => !d.status || d.status !== 'published').length} được chọn
                        </div>
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
                            ...
                            disabled={derivatives.filter(d => !d.status || d.status !== 'published').length === 0}
                          >
                            Chọn tất cả
                          </Button>
                          <Button
                          ...
                          <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                            <div className="flex items-center gap-4 text-sm">
                              ...
                              {(() => {
                                ...
                                if (derivative?.id) {
                                  ...
                                  return (
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => !isPublished && toggleDerivativeSelection(derivative.id!)}
                                ...
                                } else if (derivatives.find(d => d.platform === config.platform)) {
                                  // Show checkbox even for derivatives without IDs (newly generated)
                                  return (
                                    <div className="p-1 h-auto flex items-center">
